...
export class Dialog implements AfterContentInit, OnInit, OnDestroy {
     ...
     * @group Props
     */
    @Input() get position(): 'center' | 'top' | 'bottom' | 'left' | 'right' | 'topleft' | 'topright' | 'bottomleft' | 'bottomright' | 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right'  {
    ...
    appendContainer() {
        ...
        if (this.appendTo) {
            if (this.appendTo === 'body') this.renderer.appendChild(this.document.body, this.wrapper);
            else DomHandler.appendChild(this.wrapper.firstChild, this.appendTo);
        }
    }
